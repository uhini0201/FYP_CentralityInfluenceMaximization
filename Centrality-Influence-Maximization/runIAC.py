...
def degree_categories(G, prange):
    ...
    sorted_d = chunks(sorted(d.iteritems(), key = lambda degree: degree), len(prange))
        ...
        if not explored[node]:
            ...
            for neighbor in component:
                if not explored[neighbor]:
                    ...
                    component.update(E[neighbor].keys())
    return CC
...
def findCCs_size_distribution(G, Ep, T):
    ...
    histogram = [(s, len(list(group))) for (s, group) in groupby(sortedCCs, key = lambda size: size)]
...
if __name__ == '__main__':
    ...
    nx.write_gpickle(G, "Graph\hep.gpickle")
    print('Wrote graph G')
    print(time.time() - start)
    G = nx.read_gpickle("Graph\hep.gpickle")
    print('Read graph G')
    print(time.time() - start)
    ...
    coverage2length.sort(key=lambda l: l)
